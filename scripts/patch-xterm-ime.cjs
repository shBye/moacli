...
const replacements = [
  [
    ...
    'this._isComposing=!1,this._isSendingComposition=!1,this._compositionPosition={start:0,end:0},this._dataAlreadySent="",this._pendingCompositionSends=[],this._sentCompositionUpTo=0',
  ],
  [
    'compositionstart(){this._isComposing=!0,this._compositionPosition.start=this._textarea.value.length,this._compositionView.textContent="",this._dataAlreadySent="",this._compositionView.classList.add("active")}',
    'compositionstart(){this._isComposing=!0,this._compositionPosition.start=this._textarea.value.length,this._sentCompositionUpTo=Math.min(this._sentCompositionUpTo,this._compositionPosition.start),this._compositionView.textContent="",this._dataAlreadySent="",this._compositionView.classList.add("active")}',
  ],
  [
...
for (const [original, patched] of replacements) {
  if (runtime.includes(patched)) continue
  if (!runtime.includes(original)) throw new Error('The @xterm/xterm runtime no longer matches the reviewed IME patch target')
  runtime = runtime.replace(original, patched)
  changed = true
}

if (changed) {
  fs.writeFileSync(runtimePath, runtime)
  console.log('Applied MoaCLI Korean IME reliability patch to @xterm/xterm 5.5.0')
} else {
  console.log('MoaCLI Korean IME reliability patch is already applied')
}
